        ...
        <el-tabs v-model="activeTab" @tab-click="tabClick">
          <el-tab-pane label="客储达成进度" name="1">
            <div class="link" @click="(ev) =>  scrollLink(ev,ev.target.className)">
              <!-- node come node_ come_ card list -->
      ...
      <div class="node" id="node">
        <p class="bold">
          ...
          <el-checkbox :disabled="readonly" v-model="isCheck">
            <span style="font-size:14px">变更客储计划</span>
          </el-checkbox>
        ...
        >
          ...
          <el-table-column align="center" prop="plan_add" label="计划新增(组)">
            <template slot-scope="scope">
              ...
              <span v-else>{{scope.row.plan_add | fixed}}</span>
            </template>
          </el-table-column>
        ...
        >
          ...
          <el-table-column align="center" prop="plan_add" label="计划新增(组)">
            <template slot-scope="scope">
              ...
              ></el-input>
              <span v-else-if="scope.row.week=='完整波段' ">{{parseInt(come_sum) }}</span>
              <span v-else>{{scope.row.plan_add}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="plan_total" label="计划累计(组)">
            <template slot-scope="scope">
              <!-- <el-input v-if="scope.row.isWrite==1&&scope.row.week!=='完整波段'" v-model="scope.row.plan_total"></el-input> -->
              <span>{{ parseInt(scope.row.plan_total)}}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    delFielList(idx) {
      let namelist = ["《首开前3个月销项清单》", "《首开前3个月费用&团队》"];
      if (idx <= 1) {
