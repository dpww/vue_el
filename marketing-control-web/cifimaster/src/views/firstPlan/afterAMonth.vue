<template>
  <div class="afterAMonth">
    <navbarLink></navbarLink>
    <div class="top" v-if="isType!='view'">
      <el-row>
        <div class="right-btn">
        <el-button @click="$router.push({path:'/planWrite'})">返回列表</el-button>
        <el-button v-if="isSubmit==4" type="primary" @click="createVersions()">创建版本</el-button>
      </div>
      </el-row>
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="选择版本:">
          <el-select v-model="thisVersion" @change="switchVersion(thisVersion)" placeholder="请选择版本">
            <el-option
            ...
            <el-table-column label="操作" width="120" align="right">
              <template slot-scope="scope">
                <div class="fileBox"  v-if="scope.$index<accessoryData.length-1">
                  <el-upload
                    v-if="isSubmit!=4  && isSubmit!=3 &&!scope.row.name"
                    :on-exceed="exceed"
                    :action="action"
                    :data="{'filePath':'afterAMonth','bizID':'','orderIndex':scope.$index}"
                ...
                <div class="fileBox" :style="{height: fileHeight*40+'px'}" v-else>
                  <el-upload
                    :action="action"
                    :on-success="fileSuccess"
                    :data="{'filePath':'afterAMonth','bizID':'','orderIndex':scope.$index}"
                    ...
                    :on-error="fileError"
                  >
                    <el-button v-if="isSubmit!=4 && isSubmit!=3" type="text">+{{scope.row.handle}}</el-button>
                  </el-upload>
                </div>
              </template>
            </el-table-column>
    ...
    <div class="bottom" v-if="isSubmit!=4 && isSubmit!=3 && isType!='view'">
      <el-button type="primary" v-if="$store.state.user.userPower.includes('提交审批')" @click="saveData('submit')">提交审批</el-button>
      <el-button type="primary" @click="saveData('save')">保存</el-button>
      <el-button v-if="isShowKS"  @click="saveData('ks')" >快速审批</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fileUpload: function(event) {
      ...
      formData.append("filePath", "afterAMonth");
      ...
      subApprovePlanNode({
        ...
        if (res.code == 200) {
          if (type == "submit") {
            ...
            var hht=this.local+"/#/firstPlanPreview?BOID="+res.result.BOID+"&t=1585017217401&isKSAP=1";
            window.open(hht);
          } else {
            this.$message({
              type: "success",
    ...
    fileRemove(file, fileList) {
      ...
      console.log(file.response.data.id, "this.fileList ");
      if (file.response.data.orderIndex >= 1) {
        this.fileHeight -= 1;
      }
      delFile({ id: file.response.data.id }).then(res => {
        console.log(res);
      });
    },
