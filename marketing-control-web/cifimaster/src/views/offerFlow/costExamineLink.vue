<template>
  <div class="costExamine" ref="costExamine">
          ...
          <li>
            <el-row>
              <el-col style="text-align:right;" :span="8">
                <span style="color:red;">是否关连交易</span> :
              </el-col>
    ...
    <el-button @click="test" style="display: none;">测试</el-button>
...
import { toHistoryUrl, queryFlowId, transaction } from "@/api/offerFlow.js";
  ...
  methods: {
    useInfo() {
      costInfo({ json_id: this.$route.query.BOID }).then(res => {
        ...
        this.radio = res.result[0].isTransaction;
        ...
        if (json.DeptInfo) {
          if (
            Object.prototype.toString.call(json.DeptInfo.Item) ==
            "[object Array]"
          ) {
    ...
    window["validForm"] = function() {
      if (flag) {
        ...
        if (that.radio == "") {
          ...
          transaction({
            businessKey: window.$routeName,
            isTransaction: that.radio
          }).then(res => {
            console.log(res);
            if (res.code == 200) {
