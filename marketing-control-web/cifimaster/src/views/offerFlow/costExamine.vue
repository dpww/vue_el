        ...
        <ul class="baseInfo">
          ...
          <li>
            <el-row>
              ...
              <el-col style="padding-left:16px;" :span="16">{{value.Jbr}}</el-col>
            </el-row>
          </li>
          <!-- <li>
            <el-row>
              <el-col style="text-align:right;" :span="8">是否关联交易 :</el-col>
              <el-col style="padding-left:16px;" :span="16">
                <el-button size="mini" plain type="">是</el-button>
                <el-button size="mini" plain type="">否</el-button>
              </el-col>
            </el-row>
          </li>-->
        </ul>
...
export default {
        ...
        },
        {
          ...
          inputList: [
            ...
            },
            {
              label: "金额(不含税)",
              type: "date",
              name: "ExcludingTaxAmount_Bz"
            },
            {
              label: "进项税额",
              type: "date",
              name: "InputTaxAmount_Bz"
            },
            {
              label: "备注",
              type: "date",
  ...
  methods: {
    useInfo() {
      costInfo({ json_id: this.$route.query.BOID }).then(res => {
        ...
        let flow_code = res.result[0].flow_code;
        ...
        if (json.PayInfo) {
          if (
            Object.prototype.toString.call(json.PayInfo.Item) ==
            "[object Array]"
          ) {
        ...
        if (json.AmountInfo) {
          if (
            Object.prototype.toString.call(json.AmountInfo.Item) ==
            "[object Array]"
          ) {
            json.AmountInfo.Item.forEach(item => {
              this.value.AmountInfo.push({
                DeptGUID: item.DeptGUID,
                TaxAmount_Bz: item.TaxAmount_Bz,
                TaxRate: item.TaxRate,
                ExcludingTaxAmount_Bz: item.ExcludingTaxAmount_Bz,
                InputTaxAmount_Bz: item.InputTaxAmount_Bz,
                Remark: item.Remark,
              });
            });
          } else {
            this.value.AmountInfo = [json.AmountInfo.Item];
          }
        }
        ...
        let arrtempName = "";
        if (flow_code == "MY_FYs_CONTRACTSP") {
          arrtempName = "DeptInfo";
        } else if (flow_code == "MY_FYs_FDDZXHTSP") {
          arrtempName = "ZrrInfo";
        }
        if (json[arrtempName]) {
          if (
            Object.prototype.toString.call(json[arrtempName].Item) ==
            "[object Array]"
          ) {
            json[arrtempName].Item.forEach(item => {
            ...
            this.value.DeptInfo = [json[arrtempName].Item];
          }
        }
  ...
  mounted() {
    ...
    this.timer = setTimeout(() => {
      this.$nextTick(() => {
        if (window.cifioa) {
          window.cifioa.validForm = function() {
            var status = false;
            SendApproveCheck({
