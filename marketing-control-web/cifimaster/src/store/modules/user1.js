...
let powerMenus=localStorage.getItem('userInfo')?JSON.parse(localStorage.getItem('userInfo')).menus.pageFunctions.content[0].childMap:[];
...
const actions = {
  ...
  setPower({commit},name){
    if(powerMenus.length){
      let res=searchId(powerMenus,name);
      let {power} = res||{power:[]};
